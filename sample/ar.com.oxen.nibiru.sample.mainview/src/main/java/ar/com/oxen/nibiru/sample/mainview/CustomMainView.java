...
public class CustomMainView extends AbstractWindowViewAdapter<MainWindow>
		implements MainView {
	private static String I18N_MENU_PREFIX = "ar.com.oxen.nibiru.app.menu.";
	private Locale locale;
	private Label<String> userLabel;

	public CustomMainView(ViewFactory viewFactory,
		...
		if (appName != null) {
			...
			getAdapted().setValue(appName + " - Custom Main View");
		}
		this.userLabel = viewFactory.buildLabel(String.class);
		getAdapted().getInfoContainer().addComponent(this.userLabel);

		Label<String> customLabel = viewFactory.buildLabel(String.class);
		customLabel.setValue("This is a custom view");
		getAdapted().getInfoContainer().addComponent(customLabel);
	}
